...
namespace fly {
    ...
    void FilterPipeline::createComputePipeline() {
        VkShaderModule computeShaderModule = createShaderModule(vk.device, this->getShaderCode());
        ...
        if(vkCreateComputePipelines(vk.device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &this->pipeline) != VK_SUCCESS) {
